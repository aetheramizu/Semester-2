...
bool isGoal(int puzzle[SIZE][SIZE], int goal[SIZE][SIZE]) {
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
...
void solvePuzzle(int initial[SIZE][SIZE], int goal[SIZE][SIZE]) {
    Queue* q = createQueue();
    Queue* visited = createQueue();
    
    enqueue(q, initial, "Kondisi awal");
    enqueue(visited, initial, "");
